...
EntityGenerator.prototype.files = function files() {
  ...
  this.template('../../app/templates/src/_schema.in.rs', srcDir + 'schema.in.rs');
  this.template('../../app/templates/src/_main.rs', srcDir + 'main.rs');
  if (isNewEntity) {
    var d = new Date()
    var dateStr = '' + d.getFullYear() + (d.getMonth() + 1) + d.getDate() + d.getHours() + d.getMinutes() + d.getSeconds()
    var entityDir = migrationsDir + dateStr + '_create_' + pluralize(this.name) + '_table/'
    this.mkdir(entityDir);
    this.template('migrations/_up.sql', entityDir + 'up.sql');
    this.template('migrations/_down.sql', entityDir + 'down.sql');
  }
